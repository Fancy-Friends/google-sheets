...
/**
 * What Google Sheets actually receives.
 *
...
test("row_append sends POST /v4/spreadsheets/{spreadsheetId}/values/{range}:append", async () => {
  const { seen, transport } = capture();

  await googleSheetsRowAppend({
    config: {
