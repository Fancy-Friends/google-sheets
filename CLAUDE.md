...
npm run provider -- google-sheets            # regenerate this repo
npm run provider -- google-sheets --check    # exit 1 on any difference
```
...
| `packages/ui` | `@particle-academy/google-sheets-ui` — the authoring surface, React, every host | Generated |
| `packages/js` | `@particle-academy/google-sheets-js` — Node, on `@particle-academy/fancy-connector-core` | Generated |
| `packages/php` | `particle-academy/google-sheets-php` — PHP 8.4, on `particle-academy/fancy-connector-core` | Generated |
| `packages/py` | `fancy-google-sheets` — Python 3.11+, stdlib only | Generated |

The authoritative copy of `provider/` lives in the weaver envelope at
`providers/google-sheets/provider/`. The copy here is emitted from it and `--check`
...
| | Namespace |
|---|---|
| This repo | `github.com/Fancy-Friends/google-sheets` |
| npm | `@particle-academy/google-sheets-ui`, `@particle-academy/google-sheets-js` |
| Packagist | `particle-academy/google-sheets-php` |
| PyPI | `fancy-google-sheets` |
...
**Default to plain HTTP.** A Google Sheets SDK is third-party code and is subject to
